...
- Code: `src/transcript_intelligence/pipeline.py`
- Output: `data/processed/enriched_transcripts.csv`
- Summary: `data/processed/topic_summary.csv`
- Visualization: `reports/figures/topic_distribution.svg`
- Deck coverage: topic mix, call-type heatmap, category rationale

The approach is a hybrid taxonomy baseline: weighted real-dataset phrases, small call-type priors, evidence terms, primary/secondary scores, and a confidence margin. The taxonomy is tuned to the actual dataset: Detect reliability, Comply/audit readiness, Identity/access, Protect/backup recovery, commercial risk, roadmap gaps, onboarding/expansion, and internal execution.

## Required Task 2: Sentiment by Call Type

- Code: `src/transcript_intelligence/pipeline.py`
- Output: `data/processed/sentiment_by_call_type.csv`
- Visualization: `reports/figures/sentiment_by_call_type.svg`
- Written interpretation: `reports/executive_summary.md`
...
- Deck coverage: revenue risk queue, Detect reliability command center, roadmap evidence map, support sentiment early warning, compliance expansion lane
- Written interpretation: `reports/executive_summary.md`
...
The received dataset is a folder of 100 meeting-level JSON bundles. `scripts/prepare_real_data.py` converts those bundles into `data/raw/transcripts.csv` before analysis.
